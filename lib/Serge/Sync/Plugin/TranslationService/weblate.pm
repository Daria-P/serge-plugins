...
sub pull_ts {
    ...
    foreach my $key (keys %files) {
        ...
        my $cli_return = $self->run_weblate_cli('download --output "'.$full_path.'" '.$key, 0);
...
sub push_ts {
    ...
    foreach my $key (keys %files) {
        ...
        if (-f $full_path) {
            my $cli_return = $self->run_weblate_cli('upload --overwrite --input "'.$full_path.'" '.$key, 0);
...
sub translation_files {
    ...
    foreach my $component (@components) {
        ...
        foreach my $translation (@$json_translations_list) {
            my $language_code = $translation->{language}->{code};
            my $filename = $translation->{filename};

            if ( $language_code ~~ @$langs ) {
                $translations{$self->{data}->{project}.'/'.$component.'/'.$language_code} = $filename;
            }
        }
    }
...
sub run_weblate_cli {
    ...
    my $json_response = $self->run_cmd($command, 1);
